...
for i=1:9
    xset("color",5);
    xpoly([(d(i)/2)+0.45,a(i)/2],[b(i),b(i)]); // X
    xset("color",3);
    xpoly([a(i)/2,a(i)],[b(i),0]);             // Y
    xset("color",2);
    xpoly([a(i),(a(i)/2)+0.5],[0,c(i+1)]);     // Z
end
// Labels
xset("font",6,4);
xstring(0.5,0.9060,"$\textbf{\mathrm{SiO_{2}}}$");     // X
t=get("hdl");
t.font_foreground=5;
t.text_box_mode="centered";
xstring(0.0,-0.040,"$\textbf{\mathrm{CaO}}$");         // Y
t=get("hdl");
t.font_foreground=3;
t.text_box_mode="centered";
xstring(1.0,-0.040,"$\textbf{\mathrm{Al_{2}O_{3}}}$"); // Z
t=get("hdl");
t.font_foreground=2;
t.text_box_mode="centered";
// Data
